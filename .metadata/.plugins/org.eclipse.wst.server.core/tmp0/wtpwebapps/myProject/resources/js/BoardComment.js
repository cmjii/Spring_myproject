...
async function getauthEmailFromserver(){
    try {
...
async function spreadCommentList(bno, page=1){
    const authEmail = await getauthEmailFromserver(); 
    getCommentListFromServer(bno, page).then(result =>{
        ...
        if(result.cmtList.length > 0){
            ...
            for(let cvo of result.cmtList){
                ...
                li += `${cvo.content}`;
                li += `</div>`;
                li += `<span style="float: right;">${cvo.modAt}</span>`;
                if (authEmail === cvo.writer) {
                    li += `<button type="button" class="btn btn-outline-danger del" style="float: right; margin-right: 10px;" >삭제</button>`;
                    li += `<button type="button" class="btn btn-outline-warning mod" style="float: right; margin-right: 10px;"  data-bs-toggle="modal" data-bs-target="#myModal">수정</button>`;
                }
                li += `</li>`;
                ul.innerHTML += li;
            }
